select
  ...
  (salecheckitem.quantity * salecheckitem.price) / 100.00 as costBeforeRet,
  ((salecheckitem.quantity - coalesce(ret.quantity, 0)) * salecheckitem.price) / 100.00 as cost,
  salecheckitem.unitId,
  salecheckitem.currencyId,
  product.title as productTitle,
  unit.notation as unitNotation,
  coalesce(ret.quantity, 0) as retQuantity
from
  salecheckitem
