  ...
  (
    ...
    from
      (
        ...
        from
          (select productId, sum(quantity) / 100.0 as quantity from consignmentitem group by productId) as ip
          left join (
            select
              productId,
              sum(salecheckitem.quantity - coalesce(ret.quantity, 0)) / 100.00 as quantity
            from
              salecheckitem
            left join salecheck on salecheck.id = salecheckitem.saleCheckId
            left join (
              select saleCheckItemId, sum(quantity) / 100.00 as quantity
